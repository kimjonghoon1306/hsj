...
function renderPublicNotices() {
  ...
  if (list.length === 0) {
    container.innerHTML = '<div style="text-align:center;padding:40px;color:rgba(30,18,8,0.35);font-size:0.85rem;">등록된 공지사항이 없습니다.</div>';
    return;
  }
  ...
  container.innerHTML = sorted.map(n => `
    <div class="notice-item ${n.pinned ? 'pinned' : ''}">
      <div class="notice-left">
        <span class="notice-type-badge notice-type-${n.type}">${typeLabel(n.type)}</span>
...
function typeLabel(t) {
  return { info:'공지', event:'이벤트', notice:'긴급' }[t] || '공지';
}
...
function renderPublicMerchants() {
  ...
  if (list.length === 0) {
    grid.innerHTML = '<div style="text-align:center;padding:60px;color:rgba(30,18,8,0.35);">등록된 상인 정보가 없습니다.</div>';
    return;
  }
    ...
    <div class="merchant-pub-card">
      <div class="mpc-top">
        ...
        <div>
          ...
          <div class="mpc-cat">${m.category}${m.years ? ' · ' + m.years : ''}</div>
        </div>
      </div>
...
function submitReservation() {
  ...
  if (!name || !phone || !people || !visitDate) {
    result.innerHTML = '<div class="res-error">필수 항목을 모두 입력해주세요.</div>';
    return;
  }
  ...
  result.innerHTML = `
    <div class="res-success">
      ✅ 예약 신청이 완료되었습니다!<br>
      <span style="font-size:0.85rem;opacity:0.8;">담당자 확인 후 <strong>${phone}</strong>으로 연락드리겠습니다.</span>
    </div>`;
...
// ── 팝업 표시 ─────────────────────────────────
function initPopup() {
  if (Store.Popup.isHidden()) return;

  const popup = Store.Popup.get();
  if (!popup || popup.active === false) return;
  ...
  if (popup.imageUrl) {
    document.getElementById('sitePopupImg').innerHTML =
      `<img src="${popup.imageUrl}" style="width:100%;max-height:300px;object-fit:cover;display:block;">`;
  }
...
function closePopupWeek() {
  Store.Popup.hideForWeek();
  closePopup();
}

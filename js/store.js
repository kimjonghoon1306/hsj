...
const Popup = {
  get() {
    try { return JSON.parse(localStorage.getItem(KEYS.popup)) || null; }
    catch { return null; }
  },
  set(data) {
    localStorage.setItem(KEYS.popup, JSON.stringify(data));
  },
  clear() {
    localStorage.removeItem(KEYS.popup);
  },
  // 일주일 안보기 체크
  isHidden() {
    const until = localStorage.getItem('hsg_popup_hide_until');
    if (!until) return false;
    return Date.now() < Number(until);
  },
  hideForWeek() {
    const until = Date.now() + 7 * 24 * 60 * 60 * 1000;
    localStorage.setItem('hsg_popup_hide_until', String(until));
  }
};

// 전역 노출
window.Store = { Notices, Merchants, Reservations, Popup, initDefaultData };
